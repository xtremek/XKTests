 ...
 * the xktimer_t struct must be defined and passed to the timer_add() or the
 * timer_add_dual() function. The xktimer_t struct contains information to keep
 * track of the timer's elapsed time, running status, and a pointer to the
 * timer callback.
 *
 ...
 * Please see timer_periodic() for information on how to use this.
 *
 *
 * \section internals		Internals
 * The timer module is designed around the standard C clock() function. The
 ...
 * function.
 *
 * After dividing the ticks value by CLOCKS_PER_SEC, the timer module divides
 * again by 1000 to convert the value to milliseconds. The timer function
 * timer_clock() does this conversion and can be used by other modules to
 * keep track of time in ms independently and without using the other XKTimer
 * module functions.
 *
 * The timer module uses the ticks returned by timer_clock() to keep track of
 * the elapsed ms per timer. So, if a timer is started at a certain time, the
 * timer module gets the current ms ticks as returned by timer_clock() and adds
 * the timeout value to the internal timer ticks variable for the given timer.
 * Then, while looping through the list of registered timers, the timer module
 * then compares the current ticks with the ticks variable in the timer. If the
 * current ms ticks value is greater than the timer's ticks value, the timer
 * module will call the timer's callback function.
 *
 *
 * \section timer-single 	Single-State Timers
 * The operation of so called "single-state" timers is simple. Essentially, the
 * user creates an instance of the xktimer_t struct and passes it to the
 * timer_add() function along with the type of timer (single shot or periodic),
 * the timeout for the timer, and a callback. When the timer is started using
 * timer_start(), the XKTimer module will begin to monitor the timer until it
 * has timed out. Once this occurs, the callback function (if provided) is run,
 * and the timer is either stopped (for a single shot timer) or the timer is
 * reset and continues running (for periodic timers).
 *
 * The timeout value of single-state timers may be changed using the function
 * timer_set_timeout(). This will set the timeout for the provided timer and
 ...
 * action. The example also shows how to use the timer_task() function as well
 * as the timer_set_timeout() function.
 *
 * If a simpler or smaller type of single-state periodic timer is desired, the
 * timer_periodic() function provides this functionality. To use this function,
 * all that needs to be provided is a pointer to a ticks variable defined as
 * clock_t and a time in ms, such as 1000 ms for example. This function is
 * reentrant and is designed to be polled until it returns true. Please see the
 * section \ref timer-simple-ex for a working example of the timer_periodic()
 * function.
 *
 *
 * \section timer-dual  Dual-State Timers
 * Dual-state timers are based on basic periodic timers but have on distinction.
 ...
 *     timer_start(&timer);
 *
 *     // This is required so the timer module can call
 ...
 *       pointers.
 *
 * \todo Rename the XKTimer functions to start with xk in order to identify
 * 		 them correctly.
 *
...
/**
 * \brief		The resolution of the timer module
 *
 * This defines the resolution of the timers in the timer module. Since
...
//! A convenient define to get the size of the xktimerp_t struct
#define XKTIMER_POINTER_SIZE		sizeof(xktimer_ptr_t)
 ...
 * a time delay or "wait". This function can also run a definition of the
 * timer_wait_task() function in order to provide a way to run essential tasks
 * (such as clearing a watchdog timer) in order for the code to keep running
 * normally.
 *
 ...
 * timer_task(). For example, instead of using timer_add() to create timers and
 * using timer_task() to monitor them, the xktimer_t struct can be initialized
 * manually and then this function can be used to monitor that timer. Or, if
 * we want to disable all other timers temporarily but one or two, we can use
 ...
 * timer_add(). The timer_handle() function is used to check each timer one at
 * a time in a simple loop through the list of pointers to all active timer
 * structs.
 */
